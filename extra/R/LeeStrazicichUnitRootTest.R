ur.ls <- function(y, model = c("crash", "break"), breaks = 1, lags = NULL, thin = 1,  method = c("GTOS","Fixed"), pn = 0.1, print.results = c("print", "silent")){
  #Starttime
  starttime <- Sys.time()
  ...
  if(breaks == 1)
  {
    ...
    # One Break Case
    for(myBreak1 in myBreaks[,1][seq(1, length(myBreaks[,1]), by = thin)]){
