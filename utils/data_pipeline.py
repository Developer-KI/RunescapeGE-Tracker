...
def data_preprocess2(read: bool, filepath: str = "../data", read_path: str = "../data/processed_data.csv", write: bool = False, interp_method: str = 'linear') -> pd.DataFrame:
    ...
    # Ensure timestamp is an integer and sort for correct time-series processing
    raw_pricedata['timestamp'] = raw_pricedata['timestamp'].astype(int)
    ...
    # Filter out items with insufficient data points (less than 99% of the expected series length)
    group_raw_pricedata = raw_pricedata.groupby('item_id').nunique()
    filtered_indexes = group_raw_pricedata[group_raw_pricedata['timestamp'] < 0.99 * series_length].index
